...
const listarProdutos = async (req, res) => {
    ...
    try {
        ...
        //const query = await knex('produtos').where('usuario_id', condicao).debug
...
const obterProduto = async (req, res) => {
    ...
    try {
        //const query = await knex('produtos').where({usuario_id: $1, id: $2})
        //sei se tá certo não
...
const cadastrarProduto = async (req, res) => {
    ...
    try {
        ...
        //const query = knex('produtos').insert({usuario_id: $1, nome: $2, estoque: $3, preco: $4, categoria: $5, descricao: $6, unagen: $7})
...
const atualizarProduto = async (req, res) => {
    ...
    try {
        //const query = await knex('produtos').where({usuario_id: $1, id: $2}).debug()
        const query = `select * from produtos where usuario_id = $1 and id = $2`;
        ...
        //const queryAtualizacao = knex('produtos').where({id: $n, usuario_id: $(n+1)}).update({params.join(', ')})
...
const excluirProduto = async (req, res) => {
    ...
    try {
        // const query = knex('usuarios').where({usuario_id: $1, id: $2})
        const query = `select * from produtos where usuario_id = $1 and id = $2`;
        const { rowCount } = await conexao.query(query, [usuario.id, id]);
        ...
        //const query = knex('usuarios').where({id: $1}).del()
